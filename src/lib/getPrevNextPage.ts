...
function getPageNuber(url: string) {
  const { searchParams } = new URL(url);
  return searchParams.get("page");
}

export default function getPrevNextPage(images: ImagesResults) {
  let nextPage = images?.next_page ? getPageNuber(images.next_page) : null;

  const prevPage = images?.prev_page ? getPageNuber(images.prev_page) : null;
